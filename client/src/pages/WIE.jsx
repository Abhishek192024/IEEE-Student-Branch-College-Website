...
export default function WIE() {
  ...
  const wieAdvisors = team.filter(m => ["Mayuri Katara"].includes(m.name));
  const wieCoords = team.filter(m => ["Manyata Sharma", "Prasanta Pandey"].includes(m.name));
  ...
  const MemberCard = ({ m }) => {
    ...
    return (
      <div className="relative w-full max-w-md group bg-blue-50 dark:bg-white/5 border dark:border-white/10 rounded-2xl p-8 text-center hover:-translate-y-3 transition-all shadow-sm hover:shadow-xl">
        {m.linkedin && (
  ...
  return (
    <div className="min-h-screen mt-10 pt-0 px-6 bg-white text-gray-900 dark:bg-[#020617] dark:text-white transition-colors duration-300">
      <div className="text-center mb-20">
        <h1 className="text-4xl md:text-5xl font-bold text-blue-800 dark:text-white mb-4">IEEE – Women in Engineering (WIE)</h1>
        <p className="max-w-3xl mx-auto text-blue-700/80 dark:text-gray-400">Empowering women engineers through leadership, innovation and professional excellence.</p>
      </div>
      ...
      {wieAdvisors.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 gap-10 max-w-5xl mx-auto mb-28 place-items-center">
            {wieAdvisors.map(m => <MemberCard key={m._id} m={m} />)}
          </div>
        </>
      )}
      ...
      {wieCoords.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 max-w-5xl mx-auto mb-28">
            {wieCoords.map(m => <MemberCard key={m._id} m={m} />)}
          </div>
        </>
      )}

      <h2 className="text-3xl font-bold text-center mb-14 text-blue-800 dark:text-white">WIE Events</h2>
      <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-28">
        {events.filter((e) => e.category === "wie").map((e) => {
            const styles = {
