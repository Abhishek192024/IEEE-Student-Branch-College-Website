...
export default function MTT() {
  ...
  // 🔥 Naam se filter karenge (Narayan Vyas ka naam dono variations me diya hai safe side ke liye)
  const mttAdvisors = team.filter(m => ["Prof.(Dr.) Surendra Yadav", "Dr. Muquaddar Ali", "Narayan Vyas", "Narayan Vyas (Assistant Professor)"].includes(m.name));
  const mttCoords = team.filter(m => ["Abhishek Prasad", "Anushka Thakur"].includes(m.name));
  ...
  return (
    <div className="min-h-screen pt-10 px-6 bg-white text-gray-900 dark:bg-[#020617] dark:text-white transition-colors duration-300">
      <div className="text-center mb-20">
        <h1 className="text-4xl md:text-5xl font-bold text-blue-800 dark:text-white mb-4">IEEE – Microwave Theory & Techniques (MTT)</h1>
        <p className="max-w-3xl mx-auto text-blue-700/80 dark:text-gray-400">Advancing knowledge in RF, microwave systems, electronics and communication technologies through research and innovation.</p>
      </div>
      ...
      {mttAdvisors.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 max-w-6xl mx-auto mb-28">
            {mttAdvisors.map(m => <MemberCard key={m._id} m={m} />)}
          </div>
        </>
      )}
      ...
      {mttCoords.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 max-w-5xl mx-auto mb-28">
            {mttCoords.map(m => <MemberCard key={m._id} m={m} />)}
          </div>
        </>
      )}

      <h2 className="text-3xl font-bold text-center mb-14 text-blue-800 dark:text-white">MTT Events</h2>
      <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-28">
        {events.filter((e) => e.category === "mtt").map((e) => {
            const styles = {
